...
<div class="item">
    <div class="shell">
        <div class="header">
            ...
            <h3>{{ p.name }}</h3>
            <a href="{{ url('shop/product/' ~ p.slug) }}" class="details">voir détails</a>
        </div>
        <a href="{{ url('shop/product/' ~ p.slug) }}" class="preview-image">
        ...
        <div class="footer {{ shopState(p.state, p.quantity, 0) }}">
            ...
            <a href="{{ url('shop/product/' ~ p.slug) }}" class="btn-add">Ajouter au panier</a>
